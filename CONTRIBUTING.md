...
1. **Phân nhánh (Branching):** Không đẩy mã nguồn trực tiếp lên nhánh `main`. Luôn sử dụng nhánh tính năng `feature/WO-<issue-id>`.
2. **Nguyên tử hóa (Atomic PRs):** Mỗi PR chỉ chứa một thay đổi logic duy nhất để tránh việc Copilot sinh mã thừa.
3. **Phê duyệt (Approval):** Con người là chốt chặn cuối cùng. Yêu cầu ít nhất 01 human reviewer phê duyệt.
...
- **Kiểm soát Ảo giác (Hallucination Control):** Kỹ sư review phải kiểm tra xác nhận Copilot không sử dụng các packages, thư viện, hoặc API không tồn tại.
