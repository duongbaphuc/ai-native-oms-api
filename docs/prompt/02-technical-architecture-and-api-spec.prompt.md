...
# TASK:
Dựa trên tài liệu miền tại `docs/domain-model.md`, hãy thiết kế toàn diện hợp đồng giao tiếp API và kiến trúc bảo mật trong 3 tài liệu:
1. `docs/api-spec.md`: Đặc tả chi tiết 4 endpoint:
...
2. `docs/security-auth-spec.md`: Đặc tả cơ chế xác thực Stateless (HTTP Basic / JWT), ma trận phân quyền RBAC cho 3 vai trò: `ROLE_DISPATCHER`, `ROLE_TECHNICIAN`, `ROLE_ADMIN`.
3. `docs/api-rules.md`: Quy chuẩn trả lời lỗi chuẩn hóa theo RFC 7807 (`application/problem+json`) với định danh URN: `urn:problem-type:validation-error`, `urn:problem-type:malformed-json`, `urn:problem-type:unauthorized`, `urn:problem-type:forbidden`, `urn:problem-type:not-found`, `urn:problem-type:invalid-state-transition`, `urn:problem-type:internal-error`.
...
# DONE WHEN:
1. Tạo đầy đủ 3 tài liệu: `docs/api-spec.md`, `docs/security-auth-spec.md`, `docs/api-rules.md`.
