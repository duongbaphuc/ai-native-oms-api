...
## 1. Quản lý Secret
- **Không Hardcode:** Tuyệt đối không lưu trữ khóa bí mật (secrets), mật khẩu, hoặc token cấu hình dưới dạng chuỗi ký tự cứng trong mã nguồn. 
...
- Bắt buộc sử dụng Spring Data JPA, Hibernate, hoặc Parameterized Queries (Truy vấn tham số hóa).
- **Cấm Tuyệt Đối:** Sử dụng nối chuỗi (String concatenation) để xây dựng lệnh native SQL.

## 3. Kiểm soát Truy cập (RBAC)
- Xác minh tính toàn vẹn của mọi payload từ client.
- Định nghĩa rõ các role truy cập bằng các annotation bảo mật (ví dụ: `@PreAuthorize("hasRole('TECHNICIAN')")`).
