...
Quy tắc này cung cấp ngữ cảnh kỹ thuật (context) trực tiếp cho Copilot nhằm duy trì sự đồng nhất trong toàn bộ repo.

## 1. Định chuẩn Kỹ thuật (Technical Standards)
- **Nền tảng:** Sử dụng Java 17+ và Spring Boot 3.3+.
- **Dependency Injection:** Luôn sử dụng tiêm phụ thuộc qua hàm khởi tạo (Constructor Injection). KHÔNG sử dụng `@Autowired` trên các trường.
...
- Bắt buộc sử dụng SLF4J với cú pháp: `private static final Logger log = LoggerFactory.getLogger(ClassName.class);`.

> [!WARNING]
> **Chính sách Dữ liệu Nhạy cảm:** KHÔNG log các dữ liệu định danh cá nhân (PII) như email, số điện thoại, mật khẩu, hoặc toàn bộ payload của hệ thống.
